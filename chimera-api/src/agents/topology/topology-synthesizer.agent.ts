...
import { AgentMessage, AgentSpec, AgentTool, Incident, TopologySpec } from '../core/types';
import { ALL_TOOLS } from '../../tools/incident-tools.service';
...
export class TopologySynthesizerAgent extends BaseAgent {
    constructor(qwen: QwenClient, bus: AgentMessageBusService, correlationId: string) {
        super(
            { role: 'topology-synthesizer', name: 'TopologySynthesizer', systemPrompt: SYSTEM_PROMPT, tools: [] },
            qwen,
            bus,
            correlationId,
        );
    ...
    private buildTopologySpec(raw: any): TopologySpec {
        const agents: AgentSpec[] = (raw.agents as any[]).map((a) => ({
            role: a.role,
            name: a.name,
    ...
    private resolveTools(names: string[]): AgentTool[] {
        return names.map((n: string) => ALL_TOOLS[n]).filter(Boolean);
    }

    protected async handleMessage(_msg: AgentMessage): Promise<void> {
        // Synthesizer is invoked directly — no async messages
    }
}