...
export interface AgentMessage {
  ...
  correlationId: string; // traces entire incident lifecycle
  timestamp: number;
  retryCount?: number;
}
...
export interface TopologySpec {
  ...
  agents: AgentSpec[];
  communicationGraph: Record<string, string[]>; // agentId → [targetIds]
  rationale: string; // why this topology was chosen
}
...
export interface AgentResponse {
  agentId: string;
  content: string;
  confidence: number; // 0–1, low → triggers adversarial escalation
  timestamp: number;
}
